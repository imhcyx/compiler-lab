...
// PR003 case
static llvm::Value *EmitVectorExpr(const Expr *E) {
  assert(E->getType()->getTypeClass() == Type::ConstantArray);i
  ...
  if (BinaryOperator::classof(E)) {
    ...
    llvm::AllocaInst *AllocR = CreateTempAlloca(LTyR);
    ...
    llvm::Value *CondCmpRHS = llvm::ConstantInt::get(LTy, llvm::APInt(32, CATy->getSize()));

    // Create unsigned less-than comparison
    llvm::Value *CondCmp = Builder.CreateICmpULT(CondLoadI, CondCmpRHS, "autogenerated.for.cond.cmp");
    ...
    if (bo->getOpcode() == BO_Assign) {
      ...
      load->setAlignment(4);
      llvm::StoreInst *store = Build.CreateStore((llvm::Value*)load, LHSAddr, false);
    ...
    assert(ImplicitCastExpr::classof(E));
    const ImplicitCastExpr* Esub = dyn_cast<ImplicitCastExpr>(E);
    return LocalDeclMap.lookup((Decl*)Esub->getDecl());
  }
}
